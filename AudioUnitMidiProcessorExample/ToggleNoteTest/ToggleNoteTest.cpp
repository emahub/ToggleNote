...
int main(int argc, const char * argv[])
{
    ...
    AudioComponentDescription midiProcessorDesc{ kAudioUnitType_MIDIProcessor, ToggleNote_COMP_SUBTYPE, kAudioUnitManufacturer_Apple, 0, 0};
    
    try {
        AudioComponent midiProcessorComp = AudioComponentFindNext(nullptr, &midiProcessorDesc);
        XThrowIf(midiProcessorComp == nullptr, -1, "Couldn't find ToggleNote. Did you build the ToggleNote target?");
        XThrowIfError(AudioComponentInstanceNew(midiProcessorComp, &gMIDIProcessorUnit), "AudioComponentInstanceNew: ToggleNote");
        XThrowIfError(AudioUnitInitialize(gMIDIProcessorUnit), "AudioUnitInitialize: ToggleNote");
